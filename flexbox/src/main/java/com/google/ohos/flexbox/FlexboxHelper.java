...
import static com.google.ohos.flexbox.FlexContainer.NOT_SET;
import static com.google.ohos.flexbox.FlexDirection.ROW;
import static com.google.ohos.flexbox.FlexDirection.ROW_REVERSE;
import static com.google.ohos.flexbox.FlexItem.FLEX_BASIS_PERCENT_DEFAULT;
import static com.google.ohos.flexbox.FlexItem.FLEX_GROW_DEFAULT;
import static com.google.ohos.flexbox.FlexItem.FLEX_SHRINK_NOT_SET;
...
class FlexboxHelper {
     ...
     * {@code (long) view.getEstimatedHeight() << 32 | view.getEstimatedWidth()}
     *
     ...
     * @param orderCache 			   item order cache
     * @return an array which have the reordered indices
     */
    int[] createReorderedIndices(Component viewBeforeAdded, int indexForViewBeforeAdded,
                                 ComponentContainer.LayoutConfig paramsForViewBeforeAdded, PlainIntArray orderCache) {
     ...
     */
    int[] createReorderedIndices(PlainIntArray orderCache) {
        int childCount = mFlexContainer.getFlexItemCount();
        List<Order> orders = createOrders(childCount);
    ...
    private List<Order> createOrders(int childCount) {
        ...
        for (int i = 0; i < childCount; i++) {
            Component child = mFlexContainer.getFlexItemAt(i);
            FlexItem flexItem = (FlexItem) child.getLayoutConfig();
     ...
     */
    boolean isOrderChangedFromLastMeasurement(PlainIntArray orderCache) {
        int childCount = mFlexContainer.getFlexItemCount();
        if (orderCache.size() != childCount) {
            return true;
        }
        for (int i = 0; i < childCount; i++) {
            Component view = mFlexContainer.getFlexItemAt(i);
            if (view == null) {
                continue;
            }
            FlexItem flexItem = (FlexItem) view.getLayoutConfig();
            if (orderCache.get(i).isPresent() && flexItem.getOrder() != orderCache.get(i).get()) {
                return true;
            }
        }
        return false;
     ...
     *
     * @param result flex lines to get
     * @param widthMeasureSpec estimate specs to calculate width
     * @param heightMeasureSpec estimate specs to calculate height
     ...
     *                          list of flex lines and the child state.
     ...
     * @param existingLines     list of existing lines
     */
    ...
    void calculateFlexLines(FlexLinesResult result, int mainMeasureSpec,
        ...
        int mainMode = Component.EstimateSpec.getMode(mainMeasureSpec);
        int mainSize = Component.EstimateSpec.getSize(mainMeasureSpec);
        ...
        for (int i = fromIndex; i < childCount; i++) {
            Component child = mFlexContainer.getReorderedFlexItemAt(i);
            ...
            if (child == null) {
                ...
                continue;
            } else if (child.getVisibility() == Component.HIDE) {
                ...
                continue;
            } else if (child instanceof AbsButton) {
                evaluateMinimumSizeForCompoundButton((AbsButton) child);
            }

            FlexItem flexItem = (FlexItem) child.getLayoutConfig();

            if (flexItem.getAlignSelf() == AlignSelf.STRETCH) {
            ...
            if (flexItem.getFlexBasisPercent() != FLEX_BASIS_PERCENT_DEFAULT
                    && mainMode == Component.EstimateSpec.PRECISE) {
                childMainSize = Math.round(mainSize * flexItem.getFlexBasisPercent());
                // Use the dimension from the layout if the mainMode is not
                // EstimateSpec.EXACTLY even if any fraction value is set to
                // layout_flexBasisPercent.
                if (isMainHorizontal) {
                    child.setWidth(childMainSize);
                } else {
                    child.setHeight(childMainSize);
                }
            }
            ...
            if (isMainHorizontal) {
                ...
                new UiUtil(child.getContext()).measure(child, childMainMeasureSpec, childCrossMeasureSpec);
                ...
                childMainMeasureSpec = mFlexContainer.getChildHeightMeasureSpec(mainMeasureSpec,
                        ...
                        childMainSize);
                new UiUtil(child.getContext()).measure(child, childCrossMeasureSpec, childMainMeasureSpec);
            ...
            childState = childState | new UiUtil(child.getContext()).getMeasuredState(child);
            ...
            if (isWrapRequired(child, mainMode, mainSize, flexLine.mMainSize,
                ...
                if (isMainHorizontal) {
                    if (flexItem.getHeight() == ComponentContainer.LayoutConfig.MATCH_PARENT) {
                    ...
                    if (flexItem.getWidth() == ComponentContainer.LayoutConfig.MATCH_PARENT) {
                        // This case takes care of the corner case where the cross size of the
                        // child is affected by the just added flex line.
    ...
    /**
     * Compound buttons (ex. {{@link ohos.agp.components.Checkbox}}, {@link ohos.agp.components.ToggleButton})
     ...
     */
    private void evaluateMinimumSizeForCompoundButton(AbsButton compoundButton) {
        FlexItem flexItem = (FlexItem) compoundButton.getLayoutConfig();
        int minWidth = flexItem.getMinWidth();
        int minHeight = flexItem.getMinHeight();

        Element drawable = compoundButton.getButtonElement();
        int drawableMinWidth = drawable == null ? 0 : drawable.getMinWidth();
        int drawableMinHeight = drawable == null ? 0 : drawable.getMinHeight();
     ...
     */
    private int getViewMeasuredSizeMain(Component view, boolean isMainHorizontal) {
        if (isMainHorizontal) {
            return view.getEstimatedWidth();
        }
     ...
     */
    private int getViewMeasuredSizeCross(Component view, boolean isMainHorizontal) {
        if (isMainHorizontal) {
            return view.getEstimatedHeight();
        }
     ...
     * @see FlexContainer#setFlexWrap(FlexWrap)
     */
    private boolean isWrapRequired(Component view, int mode, int maxSize, int currentLength,
        ...
        if (mode == Component.EstimateSpec.UNCONSTRAINT) {
            return false;
        }
     ...
     */
    private void checkSizeConstraints(Component view, int index) {
        boolean needsMeasure = false;
        FlexItem flexItem = (FlexItem) view.getLayoutConfig();
        int childWidth = view.getEstimatedWidth();
        int childHeight = view.getEstimatedHeight();
        ...
        if (needsMeasure) {
            int widthSpec = Component.EstimateSpec.getSizeWithMode(childWidth, Component.EstimateSpec.PRECISE);
            int heightSpec = Component.EstimateSpec
                    .getSizeWithMode(childHeight, Component.EstimateSpec.PRECISE);
            new UiUtil(view.getContext()).measure(view, widthSpec, heightSpec);
    ...
    /**
     * Determine the main size by expanding or shrinking
     ...
     * @param widthMeasureSpec  horizontal space requirements as imposed by the parent
     * @param heightMeasureSpec vertical space requirements as imposed by the parent
     * @param fromIndex         starting item index
     * @see FlexContainer#setFlexDirection(FlexDirection)
    ...
    void determineMainSize(int widthMeasureSpec, int heightMeasureSpec, int fromIndex) {
        ...
        FlexDirection flexDirection = mFlexContainer.getFlexDirection();
        switch (mFlexContainer.getFlexDirection()) {
            ...
            case ROW_REVERSE:
                ...
                if (widthMode == Component.EstimateSpec.PRECISE) {
                    mainSize = widthSize;
                } else {
            ...
            case COLUMN_REVERSE:
                ...
                if (heightMode == Component.EstimateSpec.PRECISE) {
                    mainSize = heightSize;
                } else {
    ...
    private void expandFlexItems(int widthMeasureSpec, int heightMeasureSpec, FlexLine flexLine,
        ...
        for (int i = 0; i < flexLine.mItemCount; i++) {
            ...
            Component child = mFlexContainer.getReorderedFlexItemAt(index);
            if (child == null || child.getVisibility() == Component.HIDE) {
                continue;
            }
            FlexItem flexItem = (FlexItem) child.getLayoutConfig();
            FlexDirection flexDirection = mFlexContainer.getFlexDirection();
            if (flexDirection == ROW || flexDirection == ROW_REVERSE) {
                // The direction of the main axis is horizontal

                int childMeasuredWidth = child.getEstimatedWidth();
                ...
                int childMeasuredHeight = child.getEstimatedHeight();
                if (mMeasuredSizeCache != null) {
                ...
                if (!mChildrenFrozen[index] && flexItem.getFlexGrow() > 0f) {
                    ...
                    int childWidthMeasureSpec = Component.EstimateSpec.getSizeWithMode(newWidth,
                            Component.EstimateSpec.PRECISE);
                    new UiUtil(child.getContext()).measure(child, childWidthMeasureSpec, childHeightMeasureSpec);
                    childMeasuredWidth = child.getEstimatedWidth();
                    childMeasuredHeight = child.getEstimatedHeight();
                ...
                if (!mChildrenFrozen[index] && flexItem.getFlexGrow() > 0f) {
                    ...
                    int childHeightMeasureSpec = Component.EstimateSpec.getSizeWithMode(newHeight,
                            Component.EstimateSpec.PRECISE);
                    new UiUtil(child.getContext()).measure(child, childWidthMeasureSpec, childHeightMeasureSpec);
                    childMeasuredWidth = child.getEstimatedWidth();
                    childMeasuredHeight = child.getEstimatedHeight();
            ...
            if (flexDirection == ROW || flexDirection == ROW_REVERSE) {
                ...
                if (!mChildrenFrozen[index] && flexItem.getFlexShrink() > 0f) {
                    ...
                    int childWidthMeasureSpec =
                            Component.EstimateSpec.getSizeWithMode(newWidth, Component.EstimateSpec.PRECISE);
                    new UiUtil(child.getContext()).measure(child, childWidthMeasureSpec, childHeightMeasureSpec);
                    childMeasuredWidth = child.getEstimatedWidth();
                    childMeasuredHeight = child.getEstimatedHeight();
                    updateMeasureCache(index, childWidthMeasureSpec, childHeightMeasureSpec,
                ...
                if (!mChildrenFrozen[index] && flexItem.getFlexShrink() > 0f) {
                    ...
                    int childHeightMeasureSpec =
                            Component.EstimateSpec.getSizeWithMode(newHeight, Component.EstimateSpec.PRECISE);
                    new UiUtil(child.getContext()).measure(child, childWidthMeasureSpec, childHeightMeasureSpec);
                    childMeasuredWidth = child.getEstimatedWidth();
                    childMeasuredHeight = child.getEstimatedHeight();
                    updateMeasureCache(index, childWidthMeasureSpec, childHeightMeasureSpec,
    ...
    private int getChildWidthMeasureSpecInternal(int widthMeasureSpec, FlexItem flexItem,
        ...
        int childWidthMeasureSpec = mFlexContainer.getChildWidthMeasureSpec(widthMeasureSpec,
                ...
                flexItem.getWidth());
        int childWidth = Component.EstimateSpec.getSize(childWidthMeasureSpec);
        if (childWidth > flexItem.getMaxWidth()) {
            childWidthMeasureSpec = Component.EstimateSpec.getSizeWithMode(flexItem.getMaxWidth(),
                    Component.EstimateSpec.getMode(childWidthMeasureSpec));
        } else if (childWidth < flexItem.getMinWidth()) {
            childWidthMeasureSpec = Component.EstimateSpec.getSizeWithMode(flexItem.getMinWidth(),
                    Component.EstimateSpec.getMode(childWidthMeasureSpec));
        }
    ...
    private int getChildHeightMeasureSpecInternal(int heightMeasureSpec, FlexItem flexItem,
        ...
        int childHeightMeasureSpec = mFlexContainer.getChildHeightMeasureSpec(heightMeasureSpec,
                ...
                flexItem.getHeight());
        int childHeight = Component.EstimateSpec.getSize(childHeightMeasureSpec);
        if (childHeight > flexItem.getMaxHeight()) {
            childHeightMeasureSpec = Component.EstimateSpec.getSizeWithMode(flexItem.getMaxHeight(),
                    Component.EstimateSpec.getMode(childHeightMeasureSpec));
        } else if (childHeight < flexItem.getMinHeight()) {
            childHeightMeasureSpec = Component.EstimateSpec.getSizeWithMode(flexItem.getMinHeight(),
                    Component.EstimateSpec.getMode(childHeightMeasureSpec));
        }
     ...
     * Expand the cross size only if the height mode is EstimateSpec.EXACTLY, otherwise
     ...
     * @see FlexContainer#setAlignContent(AlignContent)
     */
    void determineCrossSize(int widthMeasureSpec, int heightMeasureSpec,
                            int paddingAlongCrossAxis) {
        // The EstimateSpec mode along the cross axis
        int mode;
        // The EstimateSpec size along the cross axis
        int size;
        FlexDirection flexDirection = mFlexContainer.getFlexDirection();
        switch (flexDirection) {
            case ROW: // Intentional fall through
            case ROW_REVERSE:
                mode = Component.EstimateSpec.getMode(heightMeasureSpec);
                size = Component.EstimateSpec.getSize(heightMeasureSpec);
                break;
            case COLUMN: // Intentional fall through
            case COLUMN_REVERSE:
                mode = Component.EstimateSpec.getMode(widthMeasureSpec);
            ...
            if (flexLines.size() == 1) {
                ...
                switch (mFlexContainer.getAlignContent()) {
                    ...
                    case SPACE_BETWEEN: {
                        ...
                        float spaceBetweenFlexLine = (float)size - totalCrossSize;
     ...
     * @see FlexContainer#setAlignItems(AlignItems)
        ...
        if (mFlexContainer.getAlignItems() == AlignItems.STRETCH) {
            ...
            for (int i = flexLineIndex, size = flexLines.size(); i < size; i++) {
                ...
                for (int j = 0, itemCount = flexLine.mItemCount; j < itemCount; j++) {
                    ...
                    Component view = mFlexContainer.getReorderedFlexItemAt(viewIndex);
                    if (view == null || view.getVisibility() == Component.HIDE) {
                        continue;
                    }
                    FlexItem flexItem = (FlexItem) view.getLayoutConfig();
                    if (flexItem.getAlignSelf() != AlignSelf.AUTO &&
                            flexItem.getAlignSelf() != AlignSelf.STRETCH) {
                        continue;
                    }
                    switch (flexDirection) {
                        case ROW: // Intentional fall through
            ...
            for (FlexLine flexLine : mFlexContainer.getFlexLinesInternal()) {
                for (Integer index : flexLine.mIndicesAlignSelfStretch) {
                    Component view = mFlexContainer.getReorderedFlexItemAt(index);
                    switch (flexDirection) {
                        case ROW: // Intentional fall through
                        case ROW_REVERSE:
     ...
     */
    private void stretchViewVertically(Component view, int crossSize, int index) {
        FlexItem flexItem = (FlexItem) view.getLayoutConfig();
        ...
        if (mMeasuredSizeCache != null) {
            ...
            measuredWidth = view.getEstimatedWidth();
        }
        childWidthSpec = Component.EstimateSpec.getSizeWithMode(measuredWidth,
                Component.EstimateSpec.PRECISE);

        int childHeightSpec = Component.EstimateSpec.getSizeWithMode(newHeight, Component.EstimateSpec.PRECISE);
        new UiUtil(view.getContext()).measure(view, childWidthSpec, childHeightSpec);
     ...
     * @param index     the index of the view
     */
    private void stretchViewHorizontally(Component view, int crossSize, int index) {
        FlexItem flexItem = (FlexItem) view.getLayoutConfig();
        ...
        if (mMeasuredSizeCache != null) {
            ...
            measuredHeight = view.getEstimatedHeight();
        }
        childHeightSpec = Component.EstimateSpec.getSizeWithMode(measuredHeight,
                Component.EstimateSpec.PRECISE);
        int childWidthSpec = Component.EstimateSpec.getSizeWithMode(newWidth, Component.EstimateSpec.PRECISE);
        new UiUtil(view.getContext()).measure(view, childWidthSpec, childHeightSpec);
        updateMeasureCache(index, childWidthSpec, childHeightSpec, view);
        mFlexContainer.updateViewCache(index, view);
    }
     ...
     * @see FlexItem#getAlignSelf()
     */
    void layoutSingleChildHorizontal(Component view, FlexLine flexLine, int left, int top, int right,
                                     int bottom) {
        FlexItem flexItem = (FlexItem) view.getLayoutConfig();
        AlignItems alignItems = mFlexContainer.getAlignItems();
        if (flexItem.getAlignSelf() != AlignSelf.AUTO) {
            ...
            alignItems = AlignItems.findByValue(flexItem.getAlignSelf().getValue());
        }
        ...
        switch (alignItems) {
            ...
            case STRETCH:
                if (mFlexContainer.getFlexWrap() != FlexWrap.WRAP_REVERSE) {
                    view.setComponentPosition(left, top + flexItem.getMarginTop(), right,
                            bottom + flexItem.getMarginTop());
                } else {
                    view.setComponentPosition(left, top - flexItem.getMarginBottom(), right,
            ...
            case BASELINE:
                if (mFlexContainer.getFlexWrap() != FlexWrap.WRAP_REVERSE) {
                    ...
                    view.setComponentPosition(left, top + marginTop, right, bottom + marginTop);
                } else {
                    int marginBottom = flexLine.mMaxBaseline - view.getEstimatedHeight() + -1;
                    marginBottom = Math.max(marginBottom, flexItem.getMarginBottom());
                    view.setComponentPosition(left, top - marginBottom, right, bottom - marginBottom);
                }
                break;
            case FLEX_END:
                if (mFlexContainer.getFlexWrap() != FlexWrap.WRAP_REVERSE) {
                    view.setComponentPosition(left,
                            top + crossSize - view.getEstimatedHeight() - flexItem.getMarginBottom(),
                    ...
                    view.setComponentPosition(left,
                            top - crossSize + view.getEstimatedHeight() + flexItem.getMarginTop(),
                            right, bottom - crossSize + view.getEstimatedHeight() + flexItem
                                    .getMarginTop());
                }
                break;
            case CENTER:
                int topFromCrossAxis = (crossSize - view.getEstimatedHeight()
                        + flexItem.getMarginTop() - flexItem.getMarginBottom()) / 2;
                if (mFlexContainer.getFlexWrap() != FlexWrap.WRAP_REVERSE) {
                    view.setComponentPosition(left, top + topFromCrossAxis,
                            right, top + topFromCrossAxis + view.getEstimatedHeight());
                } else {
                    view.setComponentPosition(left, top - topFromCrossAxis,
                            right, top - topFromCrossAxis + view.getEstimatedHeight());
                }
     ...
     * @see FlexItem#getAlignSelf()
     */
    void layoutSingleChildVertical(Component view, FlexLine flexLine, boolean isRtl,
        ...
        switch (alignItems) {
            ...
            case BASELINE:
                if (!isRtl) {
                    view.setComponentPosition(left + flexItem.getMarginLeft(), top,
                            right + flexItem.getMarginLeft(), bottom);
                } else {
                    view.setComponentPosition(left - flexItem.getMarginRight(), top,
                            right - flexItem.getMarginRight(), bottom);
                }
                break;
            case FLEX_END:
                if (!isRtl) {
                    view.setComponentPosition(
                            left + crossSize - view.getEstimatedWidth() - flexItem.getMarginRight(),
                            top,
                            right + crossSize - view.getEstimatedWidth() - flexItem.getMarginRight(),
                            bottom);
                } else {
                    // If the flexWrap == WRAP_REVERSE, the direction of the
                    // flexEnd is flipped (from left to right).
                    view.setComponentPosition(
                            left - crossSize + view.getEstimatedWidth() + flexItem.getMarginLeft(),
                            top,
                            right - crossSize + view.getEstimatedWidth() + flexItem.getMarginLeft(),
                            bottom);
                }
                break;
            case CENTER:
                ComponentContainer.LayoutConfig lp = view.getLayoutConfig();
                int leftFromCrossAxis = (crossSize - view.getEstimatedWidth()
                        + lp.getHorizontalStartMargin()
                        - lp.getHorizontalEndMargin()) / 2;
                if (!isRtl) {
                    view.setComponentPosition(left + leftFromCrossAxis, top, right + leftFromCrossAxis, bottom);
                } else {
                    view.setComponentPosition(left - leftFromCrossAxis, top, right - leftFromCrossAxis, bottom);
                }
                break;
    ...
    /**
     * Method to extract int value which consists from the lower 8 bits
    ...
    /**
     * Method to extract int value which consists from the higher 8 bits
     * @param longValue the long value that consists of width and height measure specs
    ...
    private void updateMeasureCache(int index, int widthMeasureSpec, int heightMeasureSpec,
        ...
        if (mMeasuredSizeCache != null) {
            mMeasuredSizeCache[index] = makeCombinedLong(
                    view.getEstimatedWidth(),
                    view.getEstimatedHeight());
        }
    }
    ...
    private static class Order implements Comparable<Order> {
        ...
        @Override
        public int compareTo(@NotNull Order another) {
